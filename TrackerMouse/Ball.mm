...
-(void) setHistogram:(Mat*)frame withBins:(int)bins fromCircle:(Circle*)circ {
    ...
    // Initialise an array with required number of elements for histogram bins
...
-(double) applyHistogramRatio:(Mat*)frame withBins:(int)bins withRadius:(int)radius forCentre:(cv::Point &)centreEst {
    Mat imgHist((*frame).rows, (*frame).cols, CV_64FC1);
    ...
    int ***hist = getHist(frame, bins);
    
    // Compare the frame histogram to the ball histogram
    
    double ***R = new double**[bins];
    int testVal;
    int histSum = (*frame).rows*(*frame).cols;
    int fac = histSum/initPixels; // Multiplicative factor to allow comparison of histograms of different sizes
    for (int x = 0; x < bins; ++x) {
        R[x] = new double*[bins];
        for (int y = 0; y < bins; ++y) {
...
-(bool) findCentre:(Mat*)frame inRegion:(cv::Rect) subRegion withBins:(int) bins withRadius:(int) radius inTraining:(bool) isTraining withThreshold:(double &)threshold {
    ...
    while (true) {
        ...
        // Apply detection algorithm to produce maxVal score
        
        double maxVal = [self applyHistogramRatio:&subFrame withBins:bins withRadius:radius forCentre:centreEst];
...
/*
 *  Detect whether controller is present in a frame.
 */
...
-(bool) isPresent:(Mat*)frame inRegion:(cv::Rect)subRegion withBins:(int)bins inTraining:(bool)isTraining withThreshold:(double&)threshold {
    ...
    // Apply detection algorithm to produce maxVal score and return true if maxVal exceeds threshold
